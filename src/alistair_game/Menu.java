...
public class Menu {
    ...
    void renderOptions() {
        for (int i = 0; i < choices.length; i++) {
            if (currentChoice == i) {
                OPTION_TTF.drawString((w/2) - OPTION_TTF.getWidth(choices[i])/2, i*50+250, choices[i], CHOSEN_COL);
            } else {
                OPTION_TTF.drawString((w/2) - OPTION_TTF.getWidth(choices[i])/2, i*50+250, choices[i], NOT_CHOSEN_COL);
            }
        }
    }
}
